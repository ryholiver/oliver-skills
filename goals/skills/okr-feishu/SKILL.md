...
- **模型优先**：默认读写按 `references/okr-model.json` 的字段映射执行，**不每次读表**。模板结构有变化时，先跑「模式 D · 同步数据模型」（或 `/okr-sync`）。
...
2. 按 `references/okr-model.json` 把中转站 JSON 映射到实际表字段：
...
2. 与 `references/okr-model.json` diff：
   - 新增字段 → 按语义加入模型（判断 `required/optional`）；
   - 字段改名 → 更新 `preferred_names`；
   - 字段删除 → 标记 `deprecated: true`（保留历史定义，不删）；
3. 更新 `okr-model.json`（`version+1`、`updated_at`）+ 落 `references/snapshots/snapshot-<日期>.json`；
4. 输出变化摘要（新增 / 变更 / 移除）。

## 演进原则（必读）

- **模型优先**：默认用 `okr-model.json`，不每次读表（省时）。
- **只增不改不删**：写表 / 同步都只增量；不删字段、不改类型、不覆盖已有值，历史快照保留。
- **写失败回退**：写记录报「字段不存在 / 字段名不匹配」→ 停止重试，提示用户跑 `/okr-sync`，或直接自动跑同步后重试。
- **演进钩子**：模板出现模型外的新字段（如 量化指标 / 目标值 / 当前值 / 权重）→ 自动纳入可选写入，并提示可升级进 `okr-model.json`。

## 相关

- `references/okr-model.json` — 表字段映射模型（演进单一来源，默认读写依据）
- `references/setup.sh` — 一键对齐脚本（`--sync-model` / `--ensure-fields` / `--clean-samples`）
