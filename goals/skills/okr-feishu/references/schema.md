...
- **模型优先**：默认读写以 `okr-model.json` 为准，不每次读表。本文件是**结构基线**的说明，`okr-model.json` 是可执行依据（演进单一来源）。
...
1. **模型优先**：默认用 `okr-model.json`，不每次读表（省时）。结构变化必须先同步。
2. **只增不改不删**：写表 / 同步都只增量；不删字段、不改类型、不覆盖已有值；字段消失只标记 `deprecated: true`，历史定义保留。
3. **显式同步**：模板变化（加字段/改名/换季度）时跑 `/okr-sync`（或 `setup.sh --sync-model`）→ 全量读 → diff → 更新模型 + 落快照。前期优化频繁多跑，稳定后偶尔跑。
4. **快照**：每次同步/对齐后落 `snapshots/snapshot-<日期>.json`，供 diff 与回看。
5. **演进钩子**：`--sync-model` 遇到模型外的新字段，自动纳入为 `auto_discovered` 可选字段（不破坏既有定义），并提示可升级为正式语义映射。
6. **写失败回退**：写入报「字段不存在/字段名不匹配」→ 停止重试，提示用户跑 `/okr-sync`，或直接自动跑同步后重试。
7. **对齐幂等**：`setup.sh` 可每季度复跑；`季度` 选项重复时跳过，不会重复追加。
