...
当用户进入 OKR 制定流程时，按下面顺序执行：

1. **收集资料**：先补齐周期、层级、负责人、目标背景、O、KR、权重、关键举措、基线与目标值。
2. **整理中转数据**：把收集到的信息整理成给下游消费的 JSON 中转站。
3. **生成审核文件**：把中转站内容写成 Obsidian 可直接打开的 Markdown 审核文件，保存到 `OKR/check/<周期>/<周期>-okr-review.md`。
4. **在此停下**：生成审核文件后必须停止，不得直接进入写入/同步步骤。
5. **等待用户审核**：用户在 Obsidian 中确认后，才允许进入下游写入流程。
6. **用户修改后回到第 2 步**：每次修改都重新生成审核文件，再次等待确认。
7. **用户确认后结束本轮**：确认通过后，本轮输出结束，交由下游编排层继续处理。

**停点规则：**
- **停在第 3 步之后、第 4 步之前**
- 未经用户明确确认，禁止直接写表、同步或跳过审核文件
- 审核文件是人审的唯一入口，JSON 只作为机器中转站
...
- `references/okr-review-template.md`（Obsidian 审核 Markdown 标准模板）

## 输出契约（数字中转站）

完成上述引导后，**除了对话建议，必须在末尾输出唯一一个 JSON 代码块**，作为「数字中转站」数据。该 JSON **与任何存储系统（如飞书多维表格）解耦**，下游编排层（如 `goals:okr-feishu`）直接消费它写表。

同时，审核阶段要**落到 Obsidian 的 Markdown 文件**里审阅，而不是只在对话里看人类视图：

- **审核工具**：Obsidian
- **保存目录**：`OKR/check/<周期>/`
- **文件名**：`<周期>-okr-review.md`，例如 `2026-Q3-okr-review.md`
- **用途**：人审阅、批注、留痕；JSON 继续作为机器中转站
- **模板来源**：`references/okr-review-template.md`

**结束条件：**
- 生成审核文件后，本轮对话输出视为暂告一段落
- 只有用户在 Obsidian 中确认后，才允许进入下游写入/同步
- 用户提出修改时，回到“整理中转数据 → 重新生成审核文件”
...
7. **中转站 JSON 是给下游 AI 消费的数据契约**（如 `goals:okr-feishu` 消费写表），**人审阅时不直接看 JSON**——由编排层把它写成 Obsidian 可打开的 Markdown 审核文件（`OKR/check/<周期>/<周期>-okr-review.md`），再在 Obsidian 中让用户确认；必要时也可以同时整理为卡片/表格辅助审阅。
